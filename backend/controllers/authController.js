...
export async function register(req, res, next) {
  try {
    ...
    res.status(201).json({ ok: true, user: { id: user._id.toString(), email: user.email, name: user.name, role: user.role, isPremium: user.isPremium, testsAttempted: user.testsAttempted, purchasedTests: user.purchasedTests }, accessToken, refreshToken });
  } catch (e) {
    next(e);
  }
}
...
export async function login(req, res, next) {
  try {
    ...
    const user = await User.findOne({ email }).select("+passwordHash email name role banned refreshTokens failedLoginCount lockUntil parentOf childOf isPremium testsAttempted purchasedTests");
    ...
    res.json({ ok: true, user: { id: user._id.toString(), email: user.email, name: user.name, role: user.role, isPremium: user.isPremium, testsAttempted: user.testsAttempted, purchasedTests: user.purchasedTests }, accessToken, refreshToken });
  } catch (e) {
    next(e);
  }
}
