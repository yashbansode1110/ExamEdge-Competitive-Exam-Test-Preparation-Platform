...
import { scoreAttempt } from "./resultService.js";
...
async function pickQuestionsForTest(test) {
  ...
  for (const [subject, countRaw] of Object.entries(counts)) {
    ...
    const rows = await Question.aggregate([
      { $match: { isActive: true, exam: test.exam, subject } },
      { $sample: { size: count } },
      { $project: { _id: 1 } }
    ]);
...
export async function startTest({ userId, testId, sessionId, deviceId = "" }) {
  ...
  const existing = await TestAttempt.findOne({ userId, testId, status: "in_progress" }).lean();
  if (existing) return { test, attempt: existing, resumed: true };
...
export async function submitAttempt({ userId, attemptId, sessionId, submitIdempotencyKey, now = new Date() }) {
  if (!mongoose.isValidObjectId(attemptId)) throw badRequest("Invalid attemptId", "INVALID_ID");
  const attempt = await TestAttempt.findOne({ _id: attemptId, userId });
  if (!attempt) throw notFound("Attempt not found");
  ...
  const qs = await Question.find({ _id: { $in: attempt.questionIds } })
    ...
    .lean();
  const questionsById = new Map(qs.map((q) => [q._id.toString(), q]));
  const scored = scoreAttempt({ marking: test.marking, questionsById, answers: attempt.answers });
  ...
  if (!updated) {
    ...
    return {
      ...
      result: { score: fresh?.score || 0, accuracy: fresh?.accuracy || 0, breakdown: fresh?.breakdown || {} }
    };
  }

  return { alreadySubmitted: false, timedOut, result: { score: updated.score, accuracy: updated.accuracy, breakdown: updated.breakdown } };
}
